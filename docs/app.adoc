...
<6> Jest setup with an example test for each side, server and client
<7> Java and XP library dependencies
<8> Node dependencies and the build, lint and test scripts
<9> Versions of the Enonic Market libraries the application uses
<10> Bundles both the server-side and client-side TypeScript
<11> App name, version and the required XP version
...
> Task :deploy
Deploying 'gqltutorial.jar' to '/Users/you/.enonic/sandboxes/mysandbox/home/deploy'
BUILD SUCCESSFUL
