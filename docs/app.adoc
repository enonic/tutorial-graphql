...
* in the directory `gqltutorial` setting version number to `1.0.0`
...
<2> The starter's example client-side file. We will not use this folder; the API serves its files from `static/` instead, for reasons covered in the <<graphiql#, GraphiQL chapter>>.
