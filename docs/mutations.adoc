...
The cache is created at module level, not inside a function, because a cache created per call would be empty on every call.

And the library has no way to iterate its entries, so a single key holds a map of all notes rather than one entry per note. `expire: 0` keeps them until the application is reloaded.

IMPORTANT: This is a single-server store. Each node in a cluster would keep its own copy, so requests would return different notes depending on where they landed. An application that needs to survive a restart, or to run on more than one node, should use https://developer.enonic.com/docs/xp/stable/api/lib-node[lib-node] or a grid library instead.
