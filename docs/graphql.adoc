...
In order to build your own GraphQL API on Enonic XP, you should use the https://developer.enonic.com/docs/graphql-library[GraphQL Library].
...
The starter declares dependencies through two version catalogs, so no version appears in `build.gradle` itself.
Libraries from the Enonic Market, like this one, are listed in `gradle/libs.versions.toml` and referenced as `libs.<name>`:
