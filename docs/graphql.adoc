...
Then, we must add a descriptor file named `graphql.yaml` and a controller file named `graphql.js`.
...
To do this, we need to import the GraphQL Library, as shown below.
...
[source,bash]
----
curl -X POST "http://localhost:8080/api/com.enonic.app.graphql:graphql" -H "Content-Type: application/json" -d "{ \"query\": \"{ notAField }\" }"
----
