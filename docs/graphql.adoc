...
To do this, we need to import the GraphQL Lib library in our controller, as shown below.
...
[source,js]
----
const queryType = schemaGenerator.createObjectType({ // <1>
    name: 'Query',
    fields: {
        serverTime: { // <2>
...
<1> Creates a new GraphQL schema.
This defines the overall structure of the API, starting with the root query type.
...
The complete example of the API controller is shown below.
