...
* Never keep application state in a module-level variable. XP's current engine happens to reuse one instance, but the platform gives no such guarantee and GraalJS will run many, so a module variable cannot be trusted to be shared. Notes are held in a *named* Cache Library cache: naming it makes it application-wide and shared by every script context, and the module-level variable holds a handle rather than the data. An unnamed cache is per-context and would fragment. A named cache copies values on put and get, so writes must call `put` rather than mutating a retrieved value, and its entries survive a script reload — they are cleared when the application is stopped, uninstalled or reconfigured. It remains single-server. Do not describe it as persistent or production-ready; `lib-node` or a grid library is the answer for anything real.
* Register event listeners in `src/main/resources/main.ts`, which runs once when the application loads. A listener registered in a controller or a resolver is added again on every call and never removed.
