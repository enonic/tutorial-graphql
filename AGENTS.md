...
* Notes are held in a Cache Library cache created at module level in `lib/notes.ts`. Check claims about it against the pinned version: 3.0.0 reads only `size` and `expire`, stores values by reference, and returns a cache local to the script context that created it. There is no `name` option — application-wide named caches exist only on lib-cache master (3.1.0-SNAPSHOT, added 2026-08-04 for the coming GraalJS engine pool), so do not document them as available. Nothing in the store survives a script reload, which means every `enonic dev` rebuild and every deploy empties it, and it is single-node regardless. Do not call it persistent or production-ready; `lib-node` or a grid library is the answer for anything real. On XP 8's one-engine-per-application runtime a module-level variable is in practice shared by every request, but the platform guarantees nothing, so never present that as the design.
* Register event listeners in `src/main/resources/main.ts`, which runs once when the application loads. A listener registered in a controller or a resolver is added again on every call and never removed. Known consequence: a dev rebuild that does not replace the jar reloads the scripts without re-running `main.ts`, leaving the listener bound to the previous instance of `lib/events.ts` while resolvers get a new one, so subscriptions fall silent until the app is redeployed. The subscription chapter documents this rather than moving the registration.
...
* `sse.attributes` carries the subscription document and variables from the request to the `sseEvent` handler, which receives no request of its own. Objects and numbers pass through unchanged, but an `undefined` value fails at runtime with a `NullPointerException` — omit the key instead of assigning `undefined`.
